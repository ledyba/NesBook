...
列挙するとこんな感じ。詳しく解説してもしょうがない（本当に書かれてるとおりにするだけ）ので、NES on FPGAのCPUの項目4に解説は譲ります。

|英語||
|:--|:--|
|-|cli（iフラグクリア）など、オペランドを指定する必要のない命令|
|Immediate(即値; イミディエイト)|機械語に書かれたオペランドの8ビット値をそのまま使う|
|Zero Page(ゼロページ)|上位8ビット0x00、下位8ビットが機械語に書かれたオペランドの8ビット値であるアドレスを使う（つまり、範囲は0x0000->0x00ffになります）。6502の超有名機能|
|Absolute|オペランドの示す16ビットのアドレスを使う|
|Zero Page X/Y|先述のゼロページでのアドレスに、XレジスタかYレジスタを足してつかう|
|Absolute X/Y|先述の絶対番地に、XレジスタかYレジスタを足して使う|
